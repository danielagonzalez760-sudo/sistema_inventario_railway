            ...
            <div class="section-card">
                ...
                <div class="toolbar">
                    <form method="GET" action="{{ route('admin.users.index') }}" class="search-group">
                        ...
                        <a href="{{ route('admin.users.index') }}" class="btn btn-outline">Limpiar</a>
                    </form>
                    ...
                    <div style="display:flex;gap:8px;">
                        ...
                        <a href="{{ route('admin.auditoria') }}" class="btn btn-dark">
                        ...
                        <tbody>
                            @forelse($users as $user)
                                <tr>
                                    ...
                                    <td style="color:#6b7fa3;font-size:12px;">{{ $user->email }}</td>
                                    <td>
                                        @foreach($user->roles as $role)
                                            <span class="role-badge
                                                @if($role->name === 'admin') role-admin
                                                @elseif($role->name === 'profesor') role-profesor
                                                @elseif($role->name === 'estudiante') role-estudiante
                                    ...
                                    <td class="center">
                                        <div class="action-row">
                                            <a href="{{ route('users.edit', $user) }}" class="btn-edit">✏️ Editar</a>
                                            <button @click="openModalId = {{ $user->id }}" class="btn-delete">🗑️ Eliminar</button>
                                        </div>

                                        {{-- Modal --}}
                                        <div x-show="openModalId === {{ $user->id }}" x-cloak class="modal-overlay">
